...
class MessageCleaner:
    ...
    def generate_variants(self, text: str, emojis: List[str]) -> Dict[str, str]:
        ...
        # Variant: Sentiment/Toxicity (Preserve case, emojis, and punctuation)
        # We might swap some URLs with [URL] placeholders here to keep context
        sentiment_v = URL_PATTERN.sub("[URL]", base)
        
        # Variant: Topic Analysis/Clustering (LOWERCASE, NO PUNCTUATION, NO EMOJIS)
        # Remove emojis
        ...
        return {
            ...
            "toxicity": sentiment_v, # Similar for now
            "topic": topic_v,
            "speaker": base # Original clean for dominance analysis
        }
    ...
    def process_sequence(self, raw_messages: List[RawMessage]) -> PreprocessingResult:
        ...
        # Global Participant Stats (example of what we can collect)
        df = pd.DataFrame([
            {
                "sender": p.raw.sender,
